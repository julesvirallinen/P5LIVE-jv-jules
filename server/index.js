...
function startServer(keys) {
 httpProxy
  ...
  .listen(portHTTPS, () => {
   let os = require('os');
   let ip = Object.values(os.networkInterfaces()).reduce(
    (r, list) =>
     r.concat(
      list.reduce(
       (rr, i) =>
...
function hourlyStats() {
 ...
 fs.writeFile(
  dir + '/P5L_STATS_' + new Date().getFullYear() + '-' + tPort + '.txt',
  writeStats,
  { flag: 'a+' },
  (err) => {}
 );
...
function reportStats() {
 ...
 for (let i = 0; i < Object.keys(cc).length; i++) {
  ...
  if (
   cc[ccRaw] !== undefined &&
   Object.keys(cc[ccRaw].settings.people) !== undefined
  ) {
 ...
 ccStats.nsMap.sort(function (a, b) {
  return b - a;
 });
...
if (!online) {
 app.post(
  ...
  (req, res) => {
   ...
   fs.writeFile(
    dir + '/P5L_BACKUP_' + tPort + req.body.timestamp + '.json',
    JSON.stringify(req.body.sketches, undefined, 2),
    { flag: '' },
    (err) => {}
   );
   res.send('OK');
...
const listener = server.listen(port, function () {
 if (!online) {
  console.log(
   'P5 is LIVE! visit » http://localhost:' + listener.address().port
  );
 } else {
